...
/* LOCAL MODIFICATION vs upstream 582c344: upstream defaults to DMA. This
 * project's frozen .ioc enables no I2C1 buffer DMA or NVIC vector, and adding
 * one purely to serve the display would be a CubeMX revision with no
 * functional payoff. Blocking flush costs up to ~3 ms of main-loop time per
 * frame and cannot disturb the ADC, which is triggered by TIM3 and moved by
 * DMA1_Channel1 entirely in hardware. See docs/UPSTREAM.md. */
#ifndef KK_UI_REFRESH_MODE
#define KK_UI_REFRESH_MODE KK_UI_REFRESH_BLOCKING
#endif
